...
## スキルの呼び出しとdescription

スキルが使用されるかどうかの判断は、**スキル本文（SKILL.md）の読み込み前**に行われる。

Claudeはスキルの**descriptionの内容のみ**を参照してスキルを使用するかどうかを判断するため、以下の点に注意する：

- **スキル本文に使用タイミングや条件を記述しても、呼び出し判断には影響しない**
- スキルを適切に呼び出してもらうには、descriptionに明確な使用条件を記述する必要がある
- スキル本文は、スキルが呼び出された後にClaudeが参照する作業手順書である

**descriptionに記述すべき内容:**
- このスキルがどのような場面で使われるべきか
- どのようなキーワードやタスクで呼び出されるべきか

**スキル本文（SKILL.md）に記述すべき内容:**
- スキルの具体的な実行手順
- 判断基準やルール
- 成果物の構造

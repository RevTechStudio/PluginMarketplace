...
最も類似度の高いサービスは「読書メーター」（一致率85%）です。
コンセプトと主要機能がほぼ一致しており、差別化が困難です。
...
最も類似する「読書メーター」でも一致率45%です。
独自性のあるアイディアと判断されます。
```

このあと自動的に深掘り対話（ステップ3）へ進む。

**対応の判断基準:**

- **一致率70%以上**: 高い類似性あり → ユーザーにA/B/Cの選択を求める
- **一致率40〜69%**: 中程度の類似性 → ユーザーに深掘りの承認を求める
- **一致率40%未満**: 独自性あり → 自動的に深掘りへ進む

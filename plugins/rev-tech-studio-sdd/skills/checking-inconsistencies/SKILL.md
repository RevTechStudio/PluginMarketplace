...
**例:**
- 要件定義（上位）に「パスワードは英数字混在必須」と記述
- 詳細設計（下位）にパスワード要件の記述がない
- → これは情報の欠落

**注意:** 下位ドキュメントに追加情報がある場合は、情報の追加であり、報告は不要。

### ドキュメントの階層
- **上位ドキュメント**: 要件定義、基本設計など、プロジェクトの上流工程のドキュメント
- **下位ドキュメント**: 詳細設計、実装仕様など、プロジェクトの下流工程のドキュメント
...
- ドキュメント間の情報欠落を検出する
  ...
  - 要件定義や基本設計の内容が、詳細設計や実装仕様に反映されていない箇所を特定する
...
- 上位ドキュメント（要件定義、基本設計など）の重要な情報を特定する
- 下位ドキュメント（詳細設計、実装仕様など）に同じ情報が記述されているか確認する
- 上位から下位への情報伝達が適切に行われているか確認する

以下のような情報欠落を特定する：
- 要件定義に「パスワードは英数字混在必須」と記述されているが、詳細設計にパスワード要件の記述がない
- 基本設計に「エラー時は3回までリトライ」と記述されているが、実装仕様にリトライ処理の記述がない
